...
weather_split <- initial_split(weather,
                               prop = 0.7, # percentage go to training set
                               strata = strength_gtex # use stratified sampling
                               )
...
weather_recipe <- 
  ...
  # Decorrelating
  #step_pca(all_numeric_predictors(), threshold = .8) # how of percentage to keep in PCA
...
First, let's specify:  
  ...
  - which **engine** we want to use   # engine means which package.
...
Mine is on RMSE, penalty = 1, mixture = 0, for R2, 0, 0.
